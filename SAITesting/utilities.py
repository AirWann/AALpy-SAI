...
def generate_sfa(nb_states):
    ...
    At each state, it is randomly chosen whether it is accepting, then, a random number of transitions is created (between nb_states//4 and nb_states) with random target states and random interval predicates. The intervals are generated by picking random bounds so the average size of the intervals is around 200.
    """
    states = []
