...
from aalpy.base.BooleanAlgebra import IntervalPredicate, LetterIntervalAlgebra, LetterIntervalPredicate, MonotonicAlgebra, Predicate, BooleanAlgebra, IntervalAlgebra, OrPredicate, GenIntPredicate
import numpy as np

from aalpy.utils import visualize_automaton

#simple ordered class to test non-integer intervals
...
def generate_sfa(nb_states, algebra: BooleanAlgebra = IntervalAlgebra(), element_generator = None) -> Sfa:
    ...
    At each state, it is randomly chosen whether it is accepting, then, a random number of transitions is created (between nb_states//4 and nb_states) with random target states and random interval predicates. 

    To get an sfa whose algebra is not the IntervalAlgebra, you MUST provide an element_generator function that generates random elements of the domain.
    If no element_generator is provided, the intervals are generated by picking random (int) bounds so the average size of the intervals is around 200.
    """
    if element_generator is None and not isinstance(algebra, (IntervalAlgebra, LetterIntervalAlgebra)):
        raise ValueError("If no element_generator is provided, the algebra must be IntervalAlgebra or LetterIntervalAlgebra.")
    states = []
    ...
    for i in range(nb_states):
        ...
        if isinstance(algebra, LetterIntervalAlgebra):
            for letter in algebra.alphabet:
                nb_trans = np.random.randint(3, (nb_states//len(algebra.alphabet))//2+5)
                bounds = sorted(np.random.choice(range(0, nb_trans*200), max(0, nb_trans-1), replace=False))
                bounds.append(None)
                for i, b in enumerate(bounds):
                    pred = algebra.letter_interval(letter, IntervalPredicate(int(bounds[i-1]) if i > 0 else None, int(b) if b is not None else None))
            ...
            if isinstance(algebra, IntervalAlgebra):
                ...
                bounds = sorted([element_generator() for _ in range(max(0, nb_trans-1))])
            bounds.append(None)
            for i, b in enumerate(bounds):
                if isinstance(algebra, IntervalAlgebra):
                    pred = IntervalPredicate(int(bounds[i-1]) if i > 0 else None, int(b) if b is not None else None)
                else:
                    pred = GenIntPredicate(bounds[i-1] if i > 0 else None, b if b is not None else None)
                target = np.random.choice(states)
                s.transitions.append((pred, target))
    s0 = states[0]
    sfa = Sfa(s0, states, algebra=algebra)
    return sfa
...
def sfa_to_dfa(sfa: Sfa) -> Dfa:
    ...
    for sfa_state in sfa.states:
        ...
        for predicate, target_sfa_state in sfa_state.transitions:
            ...
            dfa_state.transitions[sfa.algebra.pick_witness(predicate)] = state_mapping[target_sfa_state]


    return Dfa(state_mapping[sfa.initial_state], list(state_mapping.values()))
def dfa_to_sfa(dfa: Dfa) -> Sfa:
    """
    ...
    return Sfa(state_mapping[dfa.initial_state], list(state_mapping.values()), algebra=IntervalAlgebra())
